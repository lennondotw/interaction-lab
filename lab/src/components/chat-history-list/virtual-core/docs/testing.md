...
- `ListSimulation.layout()` models one frame: report the viewport, mount the core's render
  range, measure true heights for mounted items only, and keep the first visible item at the
  same screen position.
  It repeats until nothing new is measured and throws if that takes too many passes.
