/*
 * The five ways a glass surface can be made to appear, kept out of the `.tsx` so
 * that file exports components only.
 */

export type FadeMode = 'ancestor-opacity' | 'geometry' | 'layer-opacity' | 'mask-alpha' | 'material';

export type BackdropKind = 'checker' | 'flat' | 'stripes';

export const FADE_MODES: readonly FadeMode[] = [
  'layer-opacity',
  'mask-alpha',
  'ancestor-opacity',
  'material',
...
export const FADE_MODE_TITLE: Record<FadeMode, string> = {
  ...
  geometry: 'geometry, material at full strength',
...
export const FADE_MODE_NOTE: Record<FadeMode, string> = {
  'ancestor-opacity':
    'Worst of the five, and only broken mid-way: at any α below 1 the wrapper forms a backdrop root, so the blur has only the wrapper’s own (empty) content to sample and stops blurring entirely. At α = 1 it is correct again.',
  geometry:
    'Ship this. The material never changes; only where it is does. Cheapest to composite, and the one thing here with a physical analogue — a pane sliding into place.',
  'layer-opacity':
  ...
  material:
    'Correct. Every frame is real frost, just less of it. Note the radius saturates early — past the backdrop’s detail scale more radius changes nothing, so the tint carries the perceived ramp.',
};
